...
public class NotificationConsumer {
	...
	public void listen(final HashMap<String, Object> record, @Header(KafkaHeaders.RECEIVED_TOPIC) String topic) {
		...
		try {
			...
			if (topic.contains(configs.getCreateAssessmentTopic())
					|| topic.contains(configs.getUpdateAssessmentTopic())) {
			...
			} else if (topic.contains(configs.getSavePropertyTopic())
					|| topic.contains(configs.getUpdatePropertyTopic())) {
				...
				if (PTConstants.MUTATION_PROCESS_CONSTANT
						.contains(request.getProperty().getCreationReason().toString())) {
