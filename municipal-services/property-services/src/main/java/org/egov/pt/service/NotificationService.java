...
import static org.egov.pt.util.PTConstants.MUTATED_STRING;
...
public class NotificationService {
	...
	private String getMsgForUpdate(Property property, String msgCode, String completeMsgs,
		...
		String url = notifUtil.getShortenedUrl(notifUtil.getHost(property.getTenantId())
				.concat(configs.getViewPropertyLink().replace(NOTIFICATION_PROPERTYID, property.getPropertyId())
	...
	private String getMsgForMutation(Property property, String CompleteMsgs, String statusCode, String urlCode) {
		...
		String url = statusCode.equalsIgnoreCase(WF_STATUS_PAYMENT_PENDING) ? notifUtil.getPayUrl(property)
				: notifUtil.getMutationUrl(property);
	...
	private String getMutationUrl(Property property) {
		...
		return notifUtil.getShortenedUrl(notifUtil.getHost(property.getTenantId())
				.concat(configs.getViewMutationLink().replace(NOTIFICATION_APPID, property.getAcknowldgementNumber())
						.replace(NOTIFICATION_TENANTID, property.getTenantId())));
	}
	...
	private String getPayUrl(Property property) {
		...
		return notifUtil.getShortenedUrl(notifUtil.getHost(property.getTenantId())
				.concat(configs.getPayLink().replace(EVENT_PAY_BUSINESSSERVICE, MUTATION_BUSINESSSERVICE)
...
//			messageTemplate = messageTemplate.replace(NOTIFICATION_OWNERNAME,NOTIFICATION_EMAIL);
		...
		if (configuredChannelNames.contains(CHANNEL_NAME_EVENT)) {
			...
			List<Event> events = notifUtil.enrichEvent(smsRequests, requestInfo, property.getTenantId(), property,
					isActionReq);
			notifUtil.sendEventNotification(new EventRequest(requestInfo, events), tenantId);
		}
		if (configuredChannelNames.contains(CHANNEL_NAME_EMAIL)) {
			List<EmailRequest> emailRequests = notifUtil.createEmailRequestFromSMSRequests(requestInfo, smsRequests,
					tenantId);
	...
	private void prepareMsgAndSendToBothNumbers(PropertyRequest request, Property propertyFromSearch, String msg,
		...
		property.getOwners().forEach(owner -> {
			...
			if (uuidToMobileNumber.containsKey(owner.getUuid())
					&& uuidToMobileNumber.get(owner.getUuid()) != owner.getMobileNumber()) {

				String customizedMsg = msg.replace(PT_OWNER_NAME, owner.getName())
						.replace(PT_OLD_MOBILENUMBER, uuidToMobileNumber.get(owner.getUuid()))
						.replace(PT_NEW_MOBILENUMBER, owner.getMobileNumber());
				Map<String, String> mobileNumberToOwner = new HashMap<>();
				...
				if (configuredChannelNames.contains(CHANNEL_NAME_EVENT)) {
					...
					List<Event> events = notifUtil.enrichEvent(smsRequests, requestInfo, property.getTenantId(),
							property, isActionReq);
					notifUtil.sendEventNotification(new EventRequest(requestInfo, events), tenantId);
				}

				if (configuredChannelNames.contains(CHANNEL_NAME_EMAIL)) {
					Map<String, String> mapOfPhnoAndEmail = notifUtil.fetchUserEmailIds(mobileNumbers, requestInfo,
							request.getProperty().getTenantId());
					List<EmailRequest> emailRequests = notifUtil.createEmailRequest(requestInfo, customizedMsg,
							mapOfPhnoAndEmail);
					notifUtil.sendEmail(emailRequests, tenantId);
				}
			}
	...
	private void prepareMsgAndSendToAlternateNumber(PropertyRequest request, Property propertyFromSearch, String msg,
		...
		property.getOwners().forEach(owner -> {
			...
			if (owner.getAlternatemobilenumber() != null && !uuidToAlternateMobileNumber.get(owner.getUuid())
					.equalsIgnoreCase(owner.getAlternatemobilenumber())) {
				String customizedMsgForApp = msg.replace(PT_OWNER_NAME, owner.getName()).replace(PT_ALTERNATE_NUMBER,
						owner.getAlternatemobilenumber());
	...
	private void sendNotificationForCitizenFeedback(Property property, String localizationMsgs, String serviceType) {
		...
		String citizenFeedackMessage = notifUtil.getMsgForCitizenFeedbackNotification(property, localizationMsgs,
				serviceType);
		Map<String, String> mobileNumberToOwner = new HashMap<>();
